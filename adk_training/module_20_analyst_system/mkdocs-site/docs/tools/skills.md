...
## Skills That Write Skills — Meta Skills

System umiejętności Analyst System to nie tylko konsumpcja wiedzy — to **produkcja**.

Pipeline `generate_skill` to de facto **meta skill**: umiejętność, która tworzy nowe umiejętności. Zamyka pętlę między wiedzą rozproszoną w projekcie a strukturalnym repozytorium skilli.

### Jak to wygląda w praktyce

```
Użytkownik: "Stwórz umiejętność o saga pattern w naszych microservices"

→ Source Collector: zbiera z wiki, kodu, Jira
→ Knowledge Extractor: wyciąga wzorce, konwencje, anti-patterns
→ Dedup Checker: nie ma podobnego skilla → CREATE
→ Skill Architect: generuje SKILL.md wg agentskills.io
→ Quality Reviewer: ocena 8/10 → ✅ PASS
→ Presenter: "Nowy skill saga-patterns gotowy"

Efekt: następne dokumenty API automatycznie uwzględniają saga patterny.
```

### Dlaczego to przełomowe

| Standardowy agent AI | Analyst System z Meta Skills |
|---------------------|------------------------------|
| Stały zestaw możliwości | Rosnący zestaw — self-extending |
| Wiedza = prompt context window | Wiedza = trwałe pliki w repozytorium |
| Każda sesja od zera | Compound knowledge across sessions |
| Musisz budować nowe features ręcznie | System buduje capabilities sam |

!!! tip "Implikacja strategiczna"
    Organizacja z meta skills nie kupuje narzędzi — **buduje warsztat, który produkuje narzędzia**. Im dłużej system pracuje, tym większa przewaga nad rozwiązaniami statycznymi.

    [:material-arrow-right: Pełna koncepcja Meta Skills](../architecture/meta-skills.md)
