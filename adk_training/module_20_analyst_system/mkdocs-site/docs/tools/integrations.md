...
## Działa samodzielnie. Skaluje się z integracjami.

Analyst System **nie wymaga** żadnych integracji zewnętrznych do pracy. Pełna wartość — dokumentacja, analiza wymagań, budowanie wiedzy — jest dostępna od razu, z lokalnymi narzędziami.

Integracje z Jira, Wiki i GitLab **rozszerzają** możliwości — ale nie warunkują ich.

!!! success "Zero-dependency start"
    Zainstaluj, ustaw kontrakt projektu, zacznij pracę. MCP dodasz, gdy będziesz gotowy.
...
!!! tip "MCP jest opcjonalny — system działa w pełni bez niego"
    Integracja MCP **rozszerza** możliwości systemu o dane z Jira/Wiki/GitLab. Bez MCP system korzysta z:

    - **Lokalnych plików** (file_tools) — treść dokumentów, konteksty
    - **Wbudowanych skilli** (skill_tools) — wiedza domenowa
    - **Szablonów** (template_tools) — struktury dokumentów
    - **Kontraktu projektu** (contract) — konfiguracja, encje, moduły

    To pełny zestaw do generowania dokumentacji, analizy wymagań i budowania wiedzy. MCP dodaje dostęp do danych z systemów zespołowych — ale **nie jest warunkiem startu**.
