...
### Tryb 1 — Scaffold (bez kluczy API, natychmiast działa)

Backend zwraca placeholdery HLD/epiki — bez Gemini, bez MCP. Dobry do sprawdzenia
że UI działa i pipeline się ładuje.

```powershell
# z katalogu repo root
python -m uvicorn adk_training.module_22_spec_generator.web.app:app `
    --host 127.0.0.1 --port 8766 --log-level info
```

Otwórz http://127.0.0.1:8766 — wpisz dowolny issue key (np. `TEST-1`), kliknij `Generuj`.

### Tryb 2 — Realne Gemini (bez MCP Comarch)

Wymaga: `GOOGLE_CLOUD_PROJECT` + dostępu do Vertex AI (konto serwisowe lub `gcloud auth`).
Ticket fetcher i kontekst Jira/Wiki działają w trybie fallback (zwracają "brak MCP"),
ale **HLD + epiki generuje prawdziwy Gemini 2.5 Pro**.

```powershell
$env:SPEC_GEN_USE_REAL_PIPELINE = "1"
$env:GOOGLE_CLOUD_PROJECT        = "twoj-projekt-id"
...
> Generacja zajmuje 2–5 minut (Gemini 2.5 Pro + critique loop 3 iteracje).

### Tryb 3 — Pełny stack z React UI

Wymaga: Node.js + `node_modules` (już zainstalowane w `web/frontend/`).

```powershell
# Terminal A — backend (opcjonalnie z MCP: najpierw 01_start_mcp.ps1)
.\adk_training\module_22_spec_generator\scripts\02_start_backend.ps1

# Terminal B — frontend Vite + React (port 5173)
.\adk_training\module_22_spec_generator\scripts\03_start_frontend.ps1
```

Otwórz http://localhost:5173

### Tryb 4 — Pełny stack z Comarch MCP (Jira/Wiki/GitLab)

Wymaga: VPN Comarch + tokeny w `.env`. Kolejność uruchamiania jest **ważna**
(MCP stateful: jeden `initialize` na proces).

```powershell
# Terminal A — MCP (musi żyć razem z backendem)
.\adk_training\module_22_spec_generator\scripts\01_start_mcp.ps1

# Terminal B — backend (poczekaj aż MCP wypisze "listening")
.\adk_training\module_22_spec_generator\scripts\02_start_backend.ps1
...
Gdy coś wisi: `.\adk_training\module_22_spec_generator\scripts\00_kill_all.ps1`
i uruchom ponownie w kolejności 01 → 02 → 03.

### Testy offline

```powershell
cd adk_training/module_22_spec_generator
python -m pytest tests/test_parse_epics_json.py tests/test_prompts_and_config.py `
    tests/test_spec_generator_structure.py -q
# → 19 passed
```

### Uwagi

- **`adk web` tu nie działa** — moduł ma własny serwer FastAPI (nie ma `root_agent`
  na poziomie paczki; `agent_for_eval/agent.py` to tylko eval stub dla `AgentEvaluator`).
- Ostrzeżenia `SequentialAgent/LoopAgent/ParallelAgent is deprecated` — niegroźne,
  API jeszcze działa; migracja do `Workflow` w następnym sprincie.
- Jeśli backend wisi na `test_spec_generator_agent_e2e.py` — pomiń ten plik
  w pytest lub dodaj `--ignore=tests/test_spec_generator_agent_e2e.py`.
