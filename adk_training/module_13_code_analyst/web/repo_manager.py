...
class RepoManager:
    ...
    # Katalogi systemowe zablokowane jako repozytoria kodu
    ...
    def validate_path(path: str) -> tuple[bool, str]:
        ...
        resolved_lower = resolved.replace("/", "\\") if os.name == "nt" else resolved
        blocked = (
            RepoManager._BLOCKED_PREFIXES_WIN if os.name == "nt"
            else RepoManager._BLOCKED_PREFIXES_UNIX
        )
        for prefix in blocked:
            if resolved_lower.lower().startswith(prefix.lower()):
                return False, f"Katalog systemowy nie moze byc repozytorium: {resolved}"
