...
# Uwaga: konstruktor PlaywrightComputer akceptuje (chrome_profile_dir, headless, width, height, cdp_url).
# Cookies idą przez CHROME_PROFILE_DIR / persistent profile; nie przez kwarg.
...
class _RestrictedComputerUseToolset(ComputerUseToolset):
    """ComputerUseToolset z wykluczonymi predefiniowanymi funkcjami Vertex.

    Vertex Gemini Computer Use ma wbudowane funkcje (np. ``search`` — web search),
    ktore nie sa rejestrowane jako narzedzia ADK. Gdy model je wywola, Vertex
    zwraca ``safety_decision``, ktory ADK nie potrafi potwierdzic w odpowiedzi
    (bo nie zna tej funkcji). To powoduje 400 INVALID_ARGUMENT przy nastepnym
    wywolaniu.

    Wykluczamy te funkcje calkowicie — agent moze tylko sterowac przegladarka
    (navigate/click/type/screenshot/...), co jest dokladnie tym, czego chcemy
    dla NotebookLM.
    """

    _EXCLUDED_PREDEFINED = ["search", "open_web_browser"]

    async def process_llm_request(self, *, tool_context, llm_request) -> None:  # type: ignore[override]
        await super().process_llm_request(
            tool_context=tool_context, llm_request=llm_request
        )
        # super() dodal types.Tool(computer_use=ComputerUse(environment=...)).
        # Dorzucamy wykluczenia predefined functions.
        if llm_request.config and llm_request.config.tools:
            for tool in llm_request.config.tools:
                if isinstance(tool, genai_types.Tool) and tool.computer_use:
                    tool.computer_use.excluded_predefined_functions = list(
                        self._EXCLUDED_PREDEFINED
                    )


_toolset = _RestrictedComputerUseToolset(computer=_computer)
...
class NotebookLMAgentSystem:
    ...
    async def initialize(self) -> None:
        ...
        # Cookies/profile są obsługiwane wewnątrz PlaywrightComputer przez CONFIG.chrome_profile_dir.
        self.computer = PlaywrightComputer(
            headless=self._headless,
        )
        await self.computer.initialize()

        # 2. Utwórz ComputerUseToolset (z wykluczonymi predefined funkcjami Vertex)
        toolset = _RestrictedComputerUseToolset(computer=self.computer)
    ...
    async def ask(
        ...
        # Priorytet: self._notebook_url (jawnie przekazany) > library active > fallback
        active = self.library.get_active()
        notebook_url = self._notebook_url
        if not notebook_url and active:
