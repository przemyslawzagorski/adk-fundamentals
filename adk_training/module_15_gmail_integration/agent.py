...
# Gmail API wymaga parametru userId w ścieżce (/users/{userId}/messages).
# ADK konwertuje nazwy parametrów na snake_case (userId → user_id),
# ale LLM nie zawsze go przekazuje, więc wstrzykujemy go automatycznie.
def _inject_user_id(tool: any, args: dict, tool_context: ToolContext) -> None:
    """Automatycznie dodaje user_id='me' do callów Gmail API."""
    if tool.name.startswith("gmail_") and "user_id" not in args:
        args["user_id"] = "me"
...
root_agent = LlmAgent(
    ...
    before_tool_callback=_inject_user_id,
)
